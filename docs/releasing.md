...
The public source repository is [buaazp/feishu-im](https://github.com/buaazp/feishu-im). Version 0.2.2 currently produces a local installable tarball; npm publication is not automatic.
...
中文：源码仓库已公开，0.2.2 当前生成本地安装包。首次 npm 发布前需确认包所有权、核对安全报告渠道和元数据并完成验证。CI 不自动发布；推送和 npm 发布需明确授权。版本升级时一起验证 dsh 依赖与 Session 兼容性。
